...
import 'package:clean_architecture_2/presentation/company_list/company_list_screen.dart';
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        useMaterial3: true,
        colorScheme:
            lightColorScheme, // Material3ThemeBuilder에서 받은 color_schemes.dart 파일 안에 정의
      ),
      darkTheme: ThemeData(
        // 다크 모드 테마
        useMaterial3: true,
        colorScheme:
            darkColorScheme, // Material3ThemeBuilder에서 받은 color_schemes.dart 파일 안에 정의
      ),
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initState() {
    ...
    controller.addListener(() {
      ...
      final secondWidgetHeight =
          firstWidgetHeight + keyList[1].currentContext!.size!.height;
      final thirdWidgetHeight =
          secondWidgetHeight + keyList[2].currentContext!.size!.height;
      final forthWidgetHeight =
          thirdWidgetHeight + keyList[3].currentContext!.size!.height;
  ...
  scrollTest() {
    ...
    Scrollable.ensureVisible(keyList[count].currentContext!,
  ...
  Widget _indicator() => Row(
        ...
        children: List.generate(
            ...
            (index) => Icon(Icons.circle,
                color: count - 1 == index ? Colors.red : Colors.grey)),
      );
                    ...
                    child: Column(
                      children: [
                        ...
                        Test2(
                            ...
                            key: keyList[2],
                            question:
                                Question(question: '오늘은 뭘 먹을까요?', multiSelect: [
                              '라면',
                              '라멘',
                              '나루토',
                              '짜장면',
...
class Test2 extends StatefulWidget {
  ...
  const Test2({Key? key, required this.question, required this.writeAnswer})
      : super(key: key);
...
class _Test2State extends State<Test2> {
  ...
  void initState() {
    super.initState();
    singleSelectAnswerSheet =
        List.generate(widget.question.multiSelect.length, (index) => false);
  }
  ...
  Widget _selectWidget(String selectTitle, int index) => Row(
        children: [
          Checkbox(
              value: singleSelectAnswerSheet[index],
              onChanged: (newValue) =>
                  onChanged(newValue: newValue, index: index)),
          Text(
            selectTitle,
      ...
      child: SizedBox(
        ...
        child: Column(
          ...
          children: [
            ...
            ...List.generate(
                singleSelectAnswerSheet.length,
                (index) =>
                    _selectWidget(widget.question.multiSelect[index], index)),
          ],
        ),
      ),
    );
